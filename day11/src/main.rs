...
fn find_max_power (max_x: u32, max_y: u32, square_size: u32, grid_serial_number: u32) -> (i32, Coordinate) {
    ...
    for x in 1..(max_x-square_size) {
        for y in 1..(max_y-square_size) {
 
            let mut power: i32 = 0;
            let points = get_square(x, y, square_size);
            for point in &points {
...
fn main() {
    ...
    for size in 3..300 { 
        ...
        let (max_power, square_location) = find_max_power(300 as u32, 300 as u32, size as u32, 9110 as u32);
        if max_power_overall < max_power {
            max_power_overall = max_power;
            max_power_size = size;
            max_power_location = square_location;
        }
        println!("status update. max power is {}, at {},{},{}", max_power_overall, max_power_location.x, max_power_location.y, max_power_size);
    }
 
    println!("");
    println!("max power is {}, at {},{},{}", max_power_overall, max_power_location.x, max_power_location.y, max_power_size);
}