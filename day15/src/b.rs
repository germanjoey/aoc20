...
impl Warrior {
    fn new (id: usize, affiliation: char, location: Coordinate) -> Warrior {
        return Warrior {
            id: id, affiliation: affiliation, attack_power: 3,
            location: location, hit_points: 200
        };
    }
    ...
    fn attack (&self, opponent: &mut Warrior) -> bool {
        ...
        opponent.hit_points -= self.attack_power as i32;
...
impl Battlegrounds {
    ...
    fn find_next_move(&self, source: Coordinate, candidate_targets: &Vec<Coordinate>, blocked_cells: &mut HashSet<Coordinate>) -> Coordinate {
...
impl Battle {
    fn new (input_file: &str, elves_attack_power: usize) -> Battle {
        ...
        let mut obj = Battle {
            elves_attack_power: elves_attack_power, battlegrounds: battlegrounds,
            elves: elves, goblins: goblins
        };
        ...
        for col_index in 0..line.len() {
            ...
            if (symbol == '.') || (symbol == 'G') || (symbol == 'E') {
                ...
                if symbol != '.' {
                    ...
                    let mut warrior = Warrior::new(id, symbol, coord);

                    if symbol == 'E' {
                        warrior.attack_power = self.elves_attack_power;
    ...
    fn find_adjacent_opponents(&self, warrior: &mut Warrior, warrior_dict: &HashMap<usize, Warrior>) -> Vec<usize> {
        let mut opponent_ids: Vec<(usize, i32, Coordinate)> = vec![];
        ...
        for (other_warrior_id, other_warrior) in warrior_dict {
            ...
            if warrior.is_adjacent(other_warrior) {
                opponent_ids.push((*other_warrior_id, other_warrior.hit_points, other_warrior.location));
            }
        }
        
        opponent_ids.sort_by(|a, b| a.1.cmp(&b.1).then(a.2.cmp(&b.2)));
    ...
    fn move_warrior_to_fight(&self, warrior: &mut Warrior, warrior_dict: &mut HashMap<usize, Warrior>) {
        ...
        let mut opponent_locations: Vec<Coordinate> = vec![];

        for (_other_warrior_id, other_warrior) in warrior_dict {
            warrior_locations.insert(other_warrior.location);
            if other_warrior.affiliation != warrior.affiliation {
                opponent_locations.push(other_warrior.location);
            }
        }

        opponent_locations.sort();
...
fn main() {
    ...
    let mut expected_num_elves: usize;
    let mut attack_power: usize = 24;
    let total_rounds: usize;
    let total_remaining_hit_points: usize;

    loop {
        attack_power += 1;
        let mut battle = Battle::new(input, attack_power);
        expected_num_elves = battle.elves.len().clone();

        let (this_total_rounds, this_total_remaining_hit_points) = battle.simulate();
        if expected_num_elves == battle.elves.len() {
            total_remaining_hit_points = this_total_remaining_hit_points;
            total_rounds = this_total_rounds;
            break;
        }
    }
